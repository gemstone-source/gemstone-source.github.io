...
```
The above results shows different ports open and we can check them, lets start with `smb`
### Enumerate smb
Since `smb` service is open then we can run `smbmap` to check the files and permissions in the service
**Command**
```
┌──(gemstone㉿hashghost)-[~/…/htb/Machines/vip/active]
...
[+] IP: 10.10.10.100:445        Name: active.htb                                        
        ...
        Users                                                   NO ACCESS
```
After testing to access the above results the only promising was `Replication`
...
```
For this box the above command failed so i decided to use `smbclient` and navigate to `Replication` manually this yielded to the following:
```
...
```
I download the `Gropus.xml` file which is  `Group Policy` file with local account users information it was very common in `2008` Windows Serve. I downloaded the file into my Linux machine by using `get` command and it contains the followings:
```xml
...
┌──(gemstone㉿hashghost)-[~/…/htb/Machines/vip/active]
└─$ cat user.txt 
d0d2d6a0c7339cd43cffd98ca921ff23
```
...
12a3d6a9eba6d8d99d99b5cfc5ccf136
```
The end.
```
Mungu Nisaidie
