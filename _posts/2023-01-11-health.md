...
```
In this box only two ports were open, the `ssh` port and `webapp` port also there is another `filtered` port, port `3000`. This port seems to be handled by firewall and seems as there some sort of redirection required. Then i decide to visit the web-application hosted at port `80` and found this page:-
...
On Linux machine there is also a hit, the 
```
┌──(egovridc㉿egovridc)-[~/C7F5/htb/health/www]
└─$ python3 -m http.server 80
...
```
`netcat` returned a hit but seem it has all the outputs from my machine, for the initial step its cool but the main goal here is trying to have access to the `127.0.0.1:3000` to see what is with it.
```
┌──(egovridc㉿egovridc)-[~/C7F5/htb/health/www]
└─$ nc -nlvp 1235                                                                                                                                                                                                                         
...
```
The above result shows that the result obtained is from `http://10.10.14.18` but we want to get result form `http://127.0.0.1:3000`
### Redirection to localhost web-hook
The redirection can be performed by two methods one by using `python` and another by using `php`
...
└─$ nc -nlvp 1235
```
3. Then send the request in the web browser.

**Result**

Both methods will result to the following output.
```
┌──(egovridc㉿egovridc)-[~/C7F5/htb/health/www]                                                                                                        └─$ nc -nlvp 1235                                                                                                                                            listening on [any] 1234 ...                                                                                                                                    connect to [10.10.14.21] from (UNKNOWN) [10.10.11.176] 41024                                                                                                   POST / HTTP/1.1                                                                                                                                                Host: 10.10.14.21:1234                                                                                                                                         Accept: */*                                                                                                                                                    Content-type: application/json                                                                                                                                 Content-Length: 7733                                                                                                                                           Expect: 100-continue    
...
On searching on exploit db we will find two `txt` files which shows that the `gogs` version is vulnerable to `sqlinjection`
...
4a59adbcdba216bb4e624937ebb09a5b
```
## Root user
...
0123a54aa1778d0fc5f338d439d283d8
```
The End.
```
Mungu Nisaidie
