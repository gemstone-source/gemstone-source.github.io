...
### Gobuster Scanning.
``` 
┌──(gemstone㉿hashghost)-[~/C7F5/htb/redpanda]
...
**Command**
```
┌──(gemstone㉿hashghost)-[~/C7F5/htb/Machines/Redpanda]
...
```
┌──(gemstone㉿hashghost)-[~/C7F5/htb/openvpn]
└─$ nc -nlvp 1234          
...
## Root.
Steps for root user.
1. Checking for groups that user `woodenk` is belonging 
```
woodenk@redpanda:~$ groups
logs woodenk
```
2. Find all files that group `logs` can access
```
...
```
Interesting file is `/opt/panda_search/redpanda.log`
3. Check for symlink of the file `/opt/panda_search/redpanda.log`
```
...
```
5. Checking if the file `redpanda.log` has been used else where
```
...
1. Download  image from web `img/shy.jpg`
2. Changing the Metadata 
```
┌──(gemstone㉿hashghost)-[~/…/htb/Machines/Redpanda/www]
...
Megapixels                      : 0.518
```
3. Change the name from `shy.jpg` to `gemstone.jpg` and send it to the machine
```
┌──(gemstone㉿hashghost)-[~/…/htb/Machines/Redpanda/www]
...
```
4. Make the `xml` file you can copy it from  `/credits`
```
woodenk@redpanda:/dev/shm$ cp /credits/damian_creds.xml .
```
5. Edit the file to be like the followings:
```
...
```
7. Read the file `/dev/shm/gemstone_creds.xml` after some seconds
```
...
```
8. Take the key and create `root` file the give permission 600 then login as root
```
┌──(gemstone㉿hashghost)-[~/…/htb/Machines/Redpanda/exploit]
